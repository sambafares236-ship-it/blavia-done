...
export default function Analytics() {
  ...
  const forecast = useMemo(
    () => forecastNextDays(daily, horizon, openingBalance),
    [daily, horizon, openingBalance],
  );

  const expenseBreakdown = useMemo(
    () => categoryBreakdown(rows, "expense").slice(0, 8),
    [rows],
  );
  const incomeBreakdown = useMemo(
    () => categoryBreakdown(rows, "income").slice(0, 8),
    [rows],
  );
  ...
  const insights = useMemo(
    () => generateInsights(rows, budgets, forecast, summary),
    [rows, budgets, forecast, summary],
  );

  // Combined cashflow + forecast series (for the chart)
  const combinedChartData = useMemo(() => {
    ...
    // backfill running balance for past
    ...
    const fc = forecast.map((f) => ({
      ...
      forecast: f.forecastNet,
      balance: f.forecastBalance,
    }));
    return [...past, ...fc];
  ...
  const exportSnapshot = () => {
    ...
    const fc = forecast.map((f) => ({
      date: f.date,
      income: Math.round(f.forecastIncome),
      expenses: Math.round(f.forecastExpenses),
      net: Math.round(f.forecastNet),
      projected_balance: Math.round(f.forecastBalance),
    }));
    toCsv(
      [...out, ...fc],
      `blavia-cashflow-${format(new Date(), "yyyy-MM-dd")}.csv`,
    );
  };
    ...
    <AppShell>
      <div className="space-y-8">
        ...
        <section className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Live KPIs, cashflow predictions and insights — tuned for Kenyan
              business cycles.
            </p>
          </div>
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <KpiCard
            label="Income (30d)"
            value={fmtKsh(summary.income)}
            icon={ArrowUpRight}
            variant="success"
            loading={loading}
            hint={summary.topIncomeCategory ?? undefined}
          />
          <KpiCard
            label="Expenses (30d)"
            value={fmtKsh(summary.expenses)}
            icon={ArrowDownRight}
            variant="shield"
            loading={loading}
            hint={summary.topExpenseCategory ?? undefined}
          />
          <KpiCard
            label="Net cashflow (30d)"
            value={fmtKsh(summary.net)}
            icon={Wallet}
            variant={summary.net >= 0 ? "navy" : "warning"}
            loading={loading}
            hint={summary.net >= 0 ? "Surplus" : "Deficit"}
          />
          <KpiCard
            label="M-Pesa share"
            value={`${Math.round(summary.mpesaShare * 100)}%`}
            icon={Smartphone}
            variant="navy"
            loading={loading}
            hint={`${summary.txnCount} approved txns`}
          />
        </section>
              ...
              {loading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={combinedChartData}>
                    ...
                    />
                    <YAxis
                      tickFormatter={(v) => fmtKshCompact(Number(v))}
                      tick={{ fontSize: 11 }}
                      width={70}
                    />
                    <Tooltip
                      formatter={(v: any) => fmtKsh(Number(v))}
                      labelFormatter={(d) =>
                        format(parseISO(d as string), "EEE, d MMM yyyy")
                      }
                    />
                    ...
                    <Line
                      type="monotone"
                      dataKey="net"
                      name="Daily net (actual)"
                      stroke="hsl(213 52% 25%)"
                      strokeWidth={2}
                      dot={false}
                    />
                    <Line
        ...
        {/* Insights + Source split */}
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Brain className="h-4 w-4 text-shield" />
            ...
            <CardContent className="space-y-3">
              {loading ? (
                ...
                <p className="text-sm text-muted-foreground">
                  No insights yet — load more transactions to generate
                  recommendations.
                </p>
              ) : (
                insights.map((ins) => (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Banknote className="h-4 w-4 text-shield" />
                Income channels
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <CardHeader>
            <CardTitle className="text-base">
              ...
              <span className="text-xs font-normal text-muted-foreground">
                (budget = rolling 3-month average)
              </span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {budgets.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Need at least one prior month of data to derive budgets.
              </p>
            ) : (
