  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Employees" value={String(stats.total)} icon={Users} tone="navy" loading={loading} hint="Active" />
        <KpiCard label="On Leave" value={String(stats.onLeave)} icon={UserMinus} tone="warning" loading={loading} />
        <KpiCard label="Monthly Payroll" value={fmtKES(stats.payroll)} icon={Wallet} tone="success" loading={loading} hint="Sum of basic salaries" />
        <KpiCard label="This Month's Hires" value={String(stats.newHires)} icon={CalendarPlus} tone="success" loading={loading} />
      </section>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/40 hover:bg-muted/40">
                ...
                <TableHead className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Position</TableHead>
                <TableHead onClick={() => toggleSort("basic_salary")} className="cursor-pointer select-none text-right text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <TableBody>
              {loading && (
                <TableRow>
                  <TableCell colSpan={8} className="py-8 text-center text-sm text-muted-foreground">Loading…</TableCell>
                </TableRow>
              )}
              {!loading && pageRows.length === 0 && (
                <TableRow>
                  <TableCell colSpan={8} className="py-8 text-center text-sm text-muted-foreground">No employees match the current filters.</TableCell>
                </TableRow>
              )}
              {pageRows.map((r) => (
                <TableRow key={r.id} className="cursor-pointer"
                  ...
                  <TableCell className="text-right text-sm font-medium">{fmtKES(r.basic_salary)}</TableCell>
                  <TableCell><PaymentMethodBadge method={r.payment_method} /></TableCell>
                  ...
                  <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex justify-end gap-1">
                      ...
                      <Button size="icon" variant="ghost" title="View payslips"
            ...
            <PaginationContent>
              ...
              {Array.from({ length: pageCount }, (_, i) => i + 1).map((n) => (
                <PaginationItem key={n}>
                  <PaginationLink href="#" isActive={n === currentPage}
